...
export default function ActsListScreen() {
  ...
  const fetchGuilds = async (pageNum: number = 1, isRefresh: boolean = false) => {
    ...
    try {
      const { data, error: err } = await guildService.getActs(pageNum, 20);
    ...
    <View className={`flex-1 ${tailwind.background.both}`}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <GuildCard
            guild={item}
            onPress={() => navigation.navigate('ActDetails', { actId: item.guildId })}
          />
        )}
      ...
      {isAuthenticated && (
        <Pressable
          ...
          onPress={() => navigation.navigate('CreateAct')}
        >
