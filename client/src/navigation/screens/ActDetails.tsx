...
export default function ActDetailsScreen({ route, navigation }: Props) {
  ...
  const fetchDetails = async () => {
    ...
    try {
      const { data, error: err } = await guildService.getActById(actId);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await guildService.deleteAct(guild.guildId);
              Alert.alert('Success', 'Act deleted successfully');
              navigation.navigate('ActsList');
            } catch (err) {
              Alert.alert('Error', 'Failed to delete act');
            }
          }
        }
      ]
            ...
            {isOwner && (
              <>
                <Pressable
                  ...
                  onPress={() => navigation.navigate('EditAct', { actId: guild.guildId })}
                >
