...
export default function VenuesListScreen() {
  ...
  const fetchGuilds = async (pageNum: number = 1, isRefresh: boolean = false) => {
    ...
    try {
      const { data, error: err } = await guildService.getVenues(pageNum, 20);
    ...
    <View className={`flex-1 ${tailwind.background.both}`}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <GuildCard
            guild={item}
            onPress={() => navigation.navigate('VenueDetails', { venueId: item.guildId })}
          />
        )}
        ...
        ListEmptyComponent={
          <View className="flex-1 justify-center items-center py-20">
            ...
            <Text className={`text-base ${tailwind.textMuted.both} text-center px-10`}>
              Be the first to create a venue!
            </Text>
          </View>
        }
        ListFooterComponent={
