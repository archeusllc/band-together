...
export const ShareModal = ({
  ...
  const [permission, setPermission] = useState<"VIEW_ONLY" | "CAN_EDIT">(
    "VIEW_ONLY",
  );
  const [loading, setLoading] = useState(false);
  const [listLoading, setListLoading] = useState(false);
  const [shares, setShares] = useState<SetListShare[]>([]);
  const [generatedShare, setGeneratedShare] = useState<SetListShare | null>(
    null,
  );
  ...
  // Get the web URL for a share token (used as fallback for deep links)
  const getWebShareUrl = (shareToken: string): string => {
    // Always use Expo Hosting URL as the fallback for deep links
    // This ensures consistent behavior across all platforms and build environments
    const baseUrl = process.env.EXPO_PUBLIC_WEB_URL || "https://band-together.expo.app";
    return `${baseUrl}/setlist/shared/${shareToken}`;
  };

  // Get the share URL - returns deep link on mobile, web URL on web
  const generateShareUrl = (shareToken: string): string => {
    // On web platform, always use web URLs
    if (Platform.OS === "web") {
      return getWebShareUrl(shareToken);
    }

    // On native app (iOS/Android), use app deep link
    // Format: bandtogether://setlist/shared/[token]
    return `bandtogether://setlist/shared/${shareToken}`;
  };

  const generateQrCodeUrl = (shareToken: string): string => {
    // QR codes should always point to web URLs (work when scanned from any device)
    const webUrl = getWebShareUrl(shareToken);
  ...
  const generateNewShareWithSettings = async () => {
    ...
    try {
      ...
      setError(
        `Failed to create share: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleRevokeShare = async (shareId: string) => {
    ...
    try {
      ...
      setError(
        `Failed to revoke share: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      ...
      <View className={`flex-1 ${tailwind.background.both}`}>
        <SafeAreaView edges={["top"]} className="flex-1">
          ...
          >
            ...
            {generatedShare && qrCodeUrl && (
              <View
                className={`${tailwind.card.both} border ${tailwind.border.both} rounded-lg p-4 mb-6`}
              >
                <Text
                  className={`text-base font-semibold ${tailwind.text.both} mb-4`}
                >
                  Your Share Link
                </Text>
                ...
                {/* Share URL */}
                <View
                  className={`${tailwind.activeBackground.both} rounded-lg p-3 mb-3`}
                >
                  <Text
                    className={`text-xs font-semibold ${tailwind.textMuted.both} mb-1`}
                  >
                    SHARE URL
                  </Text>
                  <Text
                    className={`text-xs ${tailwind.text.both} mb-2 font-mono`}
                    numberOfLines={2}
                  >
                ...
                <Pressable
                  className={`py-3 rounded-lg border ${tailwind.border.both} flex-row items-center justify-center gap-2 ${
                    copyFeedback
                      ? "bg-green-500"
                      : tailwind.activeBackground.both
                  }`}
            ...
            >
                ...
                <View className="flex-row gap-2">
                  {["VIEW_ONLY", "CAN_EDIT"].map((perm) => (
                    <Pressable
                      key={perm}
                      onPress={() =>
                        setPermission(perm as "VIEW_ONLY" | "CAN_EDIT")
                      }
              ...
              >
                <Text
                  className={`text-xs ${tailwind.textMuted.both} text-center`}
                >
            ...
            <View>
              <Text
                className={`text-base font-semibold ${tailwind.text.both} mb-3`}
              >
              ...
              {listLoading ? (
                ...
                <View
                  className={`${tailwind.activeBackground.both} rounded-lg p-4 items-center`}
                >
                  <IconSymbol name="unlink" size={24} color="#9CA3AF" />
                  <Text
                    className={`text-sm ${tailwind.textMuted.both} mt-2 text-center`}
                  >
                    ...
                    >
                      <View className="flex-1">
                        <View className="flex-row items-center gap-2 mb-1">
                          <Text
                            className={`text-sm font-semibold ${tailwind.text.both}`}
                          >
                          ...
                          {share.expiresAt &&
                            new Date(share.expiresAt) > new Date() && (
                              <Text
                                className={`text-xs px-2 py-1 rounded ${tailwind.activeBackground.both} ${tailwind.textMuted.both}`}
                              >
          ...
          >
            ...
            >
              ...
              <Text className={`${tailwind.textMuted.both} mb-6`}>
                This share link will no longer work. Users with the link will
                lose access.
              </Text>
        ...
        {showQrModal && qrCodeUrl && (
          ...
          >
            ...
            >
              <View
                className={`${tailwind.card.both} rounded-lg items-center justify-between`}
                style={{
              ...
              >
                ...
                <Text
                  className={`text-xs ${tailwind.textMuted.both} mt-4 text-center`}
                >
                  Tap to close
                </Text>
              </View>
          ...
          >
            ...
            >
              ...
              >
                ...
                <Text className={`text-base ${tailwind.text.both} mb-6`}>
                  {error}
                </Text>
                <Pressable
